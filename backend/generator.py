...
def generate_workout_plan(db: Session, request: schemas.PlanRequest) -> schemas.PlanResponse:
    ...
    BANNED_KEYWORDS = ["atlas", "stone", "tire", "sled", "battling rope", "yoke", "sandbag", "prowler", "rickshaw", "car deadlift", "sledgehammer", "chain", "chains", "band ", "bands ", "stability ball", "bosu", "neck", "suspension", "trx", "board press", "guillotine", "floor press", "power snatch", "clean and jerk", "rocky", "otiz", "zercher", "handstand"]
    ...
    def find_best_exercise_for_slot(slot_dict, available_exs, used_families_this_session):
        ...
        def score(ex):
            ...
            if any(banned in name_low for banned in BANNED_KEYWORDS): s -= 2000
            if request.experience_level == schemas.ExperienceLevel.beginner:
                if any(adv in name_low for adv in ADVANCED_KEYWORDS) or any(inter in name_low for inter in INTERMEDIATE_KEYWORDS): s -= 2000
            elif request.experience_level == schemas.ExperienceLevel.intermediate:
                if any(adv in name_low for adv in ADVANCED_KEYWORDS): s -= 2000
                    
            if ex.id in used_exercise_ids: s -= 10000
    ...
    for day in range(1, request.days_per_week + 1):
        ...
        for slot in template["slots"]:
            if request.duration == schemas.WorkoutDuration.short and len(day_exercises) >= 4:
                 if slot["muscle"] in ["Biceps", "Triceps", "Brzuch", "Barki"] and len(day_exercises) > 4: continue
