...
export default function Home() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (data.days) {
        data.days = data.days.map((day: WorkoutDayResponse, dIdx: number): GeneratedWorkoutDay => ({
          ...day,
          exercises: day.exercises.map((ex: Exercise, eIdx: number) => ({
            ...ex,
            instance_key: `day-${dIdx}-ex-${eIdx}-${ex.id}`
          })),
          warmup: (day.warmup || []).map((ex: Exercise, eIdx: number) => ({
            ...ex,
        ...
        <section className="bg-white/[0.03] p-6 sm:p-12 md:p-16 rounded-[40px] sm:rounded-[60px] shadow-2xl border border-white/10 backdrop-blur-3xl relative overflow-hidden group">
          ...
          <form onSubmit={handleSubmit} className="space-y-10 relative z-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
              ...
              ] satisfies Array<FormSelectField | FormNumberField>).map((f) => (
